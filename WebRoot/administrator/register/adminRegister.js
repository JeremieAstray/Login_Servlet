...
{
	flag = 1; 
	if(!checkadminName(document.form.adminName.value.toLowerCase()))
		flag = 0; 
	if(!checkTrueName(document.form.TrueName.value))
		flag = 0; 
	...
	if(!Checkshortnum(document.form.shortNum.value))
		flag = 0; 
	if(!checkaddress(document.form.address.value))
		flag = 0;
...
//===========================================华丽的分割线================================================
function checkadminName(ssn)
{
	if( ssn.length<3 || ssn.length>18 ) {
		document.getElementById("adminNameErr").innerHTML = "<font color='red'>请输入正确的用户名,用户名长度为3-18位！</font>";
		return false;
	}
	else if (isWhiteWpace(ssn)){
		document.getElementById("adminNameErr").innerHTML = "<font color='red'>请输入正确的用户名,用户名中不能包含空格！</font>";
		return false;
	}
	else if (!isSsnString(ssn)){
		document.getElementById("adminNameErr").innerHTML = "<font color='red'>对不起，用户名只能是英文，数字，标点组成</font>";
		return false;
	}else if(ajax("../../checkUser.jsp?type=admin&name="+ssn)==0)
		{
		document.getElementById("adminNameErr").innerHTML = "<font color='red'>对不起，该用户名已被注册</font>";
		return false;
		}
	document.getElementById("adminNameErr").innerHTML = "";
	return true;
}
//===========================================华丽的分割线================================================
function checkTrueName(ssn){
	if(ssn == ""){
		document.getElementById("TrueNameErr").innerHTML = "<font color='red'>请输入姓名！</font>";
		return false;
	}
	document.getElementById("TrueNameErr").innerHTML = "";
	return true;
}
...
function checkPassword(ssn){
	...
	else if( document.form.adminName.value == ssn ) {
...
function checkPasswordre(ssn){
	if( ssn =="" ) {
		document.getElementById("passwordreErr").innerHTML = "<font color='red'>请输入密码确认！</font>";
		return false;
	}
	else if( ssn != document.form.password.value ) {
		document.getElementById("passwordreErr").innerHTML = "<font color='red'>两次密码输入不一致！</font>";
		return false;
	}
...
function  Checkqq(qq){
	if(qq == ""){
		document.getElementById("qqErr").innerHTML = "<font color='red'>请输入qq号!</font>";
		return false;
	}
	else if(isnum(qq)){
...
function  Checklongnum(num){
	if(num == ""){
		document.getElementById("longNumErr").innerHTML = "<font color='red'>请输入长号!</font>";
		return false;
	}
...
function  Checkshortnum(num){
	if(num == ""){
		document.getElementById("shortNumErr").innerHTML = "<font color='red'>请输入短号!</font>";
		return false;
	}
	else if(isnum(num)){
...
function  checkaddress(ssn){
	if(ssn == ""){
		document.getElementById("addressErr").innerHTML = "<font color='red'>请输入宿舍地址</font>";
		return false;
	}
